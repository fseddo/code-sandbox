...
import { runTestsClient } from "@/problems/algo/tester/runTestsClient";

/**
 * The grading lifecycle for one problem: grades a buffer in a terminable browser Web Worker
 * (`runTestsClient` — no server round-trip), holds the latest `outcome`, tracks which mode is in flight,
 * and captures the passing buffer on a Submit that clears every case (the algo completion oracle).
 * `submittedSolution` is the answer-of-record — distinct from the working draft in `useAlgoEditor`;
 * restoring it back into the editor is the root's job, not this hook's.
 */
export const useAlgoSubmission = (problem: ClientProblem) => {
  ...
  const run = async (mode: RunMode, buffer: CompletedSolution) => {
    ...
    try {
      const result = await runTestsClient({ problem, language: buffer.language, source: buffer.source, mode });
      setOutcome(result);
      ...
      setOutcome({ status: "crashed", message: error instanceof Error ? error.message : "Grading failed." });
    } finally {
      setRunningMode(null);
    }
  };
