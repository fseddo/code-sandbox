 ...
 */
export type Problem<Args extends unknown[] = unknown[], Result = unknown> = ProblemBase & {
  kind: "algo";
 ...
 * type-checked against it: `defineProblem<[number[], number], number[]>({ … })`. `kind: "algo"` is
 * injected here, so authored modules never spell it out.
 */
export const defineProblem = <Args extends unknown[], Result>(
  problem: Omit<Problem<Args, Result>, "kind">,
): Problem<Args, Result> => ({ kind: "algo", ...problem });

/** Author a build problem. `kind: "build"` is injected, mirroring `defineProblem`. */
