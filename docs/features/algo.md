...
The LeetCode side: pick a problem, write a solution in JS or TS, and run it against the problem's test cases — graded **client-side in a terminable browser Web Worker** (no server round-trip; the historical `/api/judge` route is gone). All code lives under [`src/problems/`](../../src/problems/) (UI + data + tester) and [`src/app/problems/`](../../src/app/problems/) (the problem route). The problem *list* lives in the catalog on the home page — see [navigation.md](navigation.md). The submission editor is the shared [`CodeEditor`](../../src/components/CodeEditor.tsx).
...
type AlgoProblem<Args extends unknown[] = unknown[], Result = unknown> = {
  ...
  hiddenTests: TestCase<Args, Result>[]; // Submit adds them; with client-side grading they ship to the browser
  io?: ProblemIo; checker?: string;       // see below
};
```

[`defineAlgoProblem<Args, Result>(…)`](../../src/problems/data/problem.ts) pins the signature so authoring a problem type-checks each case's `args` tuple and `expected` against it — see [twoSum.ts](../../src/problems/data/problems/twoSum.ts) (`defineAlgoProblem<[number[], number], number[]>`). Get the signature wrong in a case and it's a compile error, not a runtime surprise. There is no separate `tests` field: the run set is built from `examples` (plus `hiddenTests` on Submit) by `testsForMode` in [testPlan.ts](../../src/problems/algo/tester/testPlan.ts). `TopicTag` is a single-source literal union of the catalog's topic slugs; `source` carries provenance (`origin`, `frontendId`, `acRate`, authoring `confidence`).
...
- **`checker`** (JS arrow-source `(actual, args, expected) => boolean`, authored/trusted) replaces deep-equal for problems with multiple valid answers. `args` is passed **post-call** (after the function returns), so a checker can score an **in-place** mutation of the input or assert the result is the same instance (`actual === args[0]`). See [longestPalindrome.ts](../../src/problems/data/problems/longestPalindrome.ts) (multi-answer) and [sortColors.ts](../../src/problems/data/problems/sortColors.ts) (in-place).

[`problems/index.ts`](../../src/problems/data/problems/index.ts) is the registry: a `Record<id, AnyProblem>` built from **literal-string keys** with `satisfies`, so `ProblemId = keyof typeof problems` is a real union of the ids (not `string`) and the authored modules keep their precise generics while the registry erases them to `AnyProblem` (it holds heterogeneous signatures and both kinds). A drift-guard test asserts each key equals its module's `.id`. `getProblem(id)` / `listProblems()` are the read API (returning `AnyProblem`) — problem *info* needs no HTTP route; server components import the registry directly. The algo-only boundary — the [`/problems/[id]` page](../../src/app/problems/[id]/page.tsx) — narrows on `kind === "algo"` before touching algo-specific fields, handing the workspace an already-narrowed `AlgoProblem` (`ClientProblem`) that the client grader runs against. Adding a problem = a new module under `problems/` + one line in `index.ts`; the full authoring rubric (and the `problem-importer` agent that runs it) live in [problem-authoring.md](problem-authoring.md). Two gates run every reference solution through the real worker: [`scripts/verifyProblems.mjs`](../../scripts/verifyProblems.mjs) (standalone, optional `[slug]` filter) and the `npm test` vitest suite ([problems.test.ts](../../src/problems/data/problems/problems.test.ts)); the io converters ([io.mjs](../../src/problems/algo/tester/io.mjs)) also have unit tests.

`SubmissionOutcome` (also in `problem.ts`) is the discriminated union the tester and UI share: `ok` | `compile-error` | `timeout` | `crashed`. It's the contract between the grading core ([grade.mjs](../../src/problems/algo/tester/grade.mjs)) and [ResultsPanel](../../src/problems/algo/ResultsPanel.tsx).

## Execution — a terminable browser Web Worker

Grading runs **entirely on the client**, in a dedicated browser Web Worker — see [runTestsClient.ts](../../src/problems/algo/tester/runTestsClient.ts) and [judge.worker.ts](../../src/problems/algo/tester/judge.worker.ts). There is no server round-trip and no server-side code-execution surface (the historical `/api/judge` route is gone). The grading logic itself is an **isomorphic core**, [grade.mjs](../../src/problems/algo/tester/grade.mjs), shared verbatim between the browser worker (the live judge) and a Node `worker_threads` worker ([problemTester.worker.mjs](../../src/problems/algo/tester/problemTester.worker.mjs)) that only the test/verify tooling uses.

Flow:

1. [`useAlgoSubmission`](../../src/problems/algo/useAlgoSubmission.ts) calls [`runTestsClient`](../../src/problems/algo/tester/runTestsClient.ts) with `{ problem, language, source, mode }`.
2. [`runTestsClient`](../../src/problems/algo/tester/runTestsClient.ts) spawns a `new Worker(new URL("./judge.worker.ts", import.meta.url))`, posts it `{ source, language, functionName, tests, io, checker }` (the `tests` built by `testsForMode` in [testPlan.ts](../../src/problems/algo/tester/testPlan.ts)), and **races the worker's reply against a per-mode `setTimeout`**. On overrun it calls `worker.terminate()` and resolves `{ status: "timeout" }`.
3. [`judge.worker.ts`](../../src/problems/algo/tester/judge.worker.ts) hands the request to [`gradeSubmission`](../../src/problems/algo/tester/grade.mjs). The core transpiles TS→JS with **sucrase** (type-strip only, no type-checking — what a test runner wants), extracts the named function, runs each test (hydrating `io` params, flattening an `io` result), compares via the `checker` when present (handed the post-call `args` so it can judge in-place mutations) else **deep-equals** `actual` vs `expected`, and returns `{ status: "ok", results }`.

### Why a Web Worker, and why terminate is the point

A bare `setTimeout` on the main thread can't interrupt a synchronous `while(true){}` — JS is single-threaded, so the timer callback never runs while the loop holds the thread. Running the submission **off the main thread** in a Worker is what makes the loop interruptible: `worker.terminate()` forcibly destroys the thread, catching sync hangs and async hangs (`await new Promise(() => {})`) alike, and keeping a heavy submission from freezing the UI. Each test `structuredClone`s its args so one case's mutation can't leak into the next. The budget is the mode's wall-clock limit — Run 2000 ms, Submit 8000 ms — held in `WALL_CLOCK_LIMIT_MS` in [testPlan.ts](../../src/problems/algo/tester/testPlan.ts) and shared by both runners so it can't drift between them.

### One isomorphic core, two transports

[grade.mjs](../../src/problems/algo/tester/grade.mjs) deliberately uses `new Function` rather than `node:vm`, so the *same* grader runs in the browser (no `node:` builtins) and in Node. The two thin wrappers add only transport + termination: [judge.worker.ts](../../src/problems/algo/tester/judge.worker.ts) over the browser `Worker` message channel, [problemTester.worker.mjs](../../src/problems/algo/tester/problemTester.worker.mjs) over Node `worker_threads`; [runTestsClient.ts](../../src/problems/algo/tester/runTestsClient.ts) and [runTests.ts](../../src/problems/algo/tester/runTests.ts) are the matching spawners. The Node path exists **only** for the offline gates — `npm test` ([problems.test.ts](../../src/problems/data/problems/problems.test.ts)) and [verifyProblems.mjs](../../scripts/verifyProblems.mjs) / [verifyGuideTestCases.mjs](../../scripts/verifyGuideTestCases.mjs) — so a broken problem fails CI without a browser. No app code imports it.

### Function extraction handles both declaration forms

The core wraps the user source in a function body and returns the named function:

```js
new Function("ListNode", "TreeNode", "console",
  `${code}\n; return typeof ${fn} === "function" ? ${fn} : undefined;`)
```

Wrapping in one body means both `function twoSum(){}` (hoisted) and `const twoSum = () => {}` (lexical) resolve. Globals (`ListNode`, `TreeNode`, a `console` capturer) are injected as **parameters**, not a shared global object — no `vm` context to manage.

### Security & integrity posture

- **No server-side code execution.** Every submission runs in the user's own browser tab, so untrusted code never reaches a server — the whole server-side RCE / sandbox-escape class is gone, and the app ships as a static client bundle. `new Function` evaluating the user's *own* code in their *own* tab is not a cross-user risk.
- **Hidden tests and the reference solution ship to the client.** Client-side grading needs the full problem in the browser, so `hiddenTests` (and any `checker` / reference solution) sit in the page bundle — readable via devtools. This is the deliberate trade for deleting the server: it weakens *answer secrecy*, not safety. Restoring secrecy (a server gate that holds the hidden set back) is a documented future step; the `runTestsClient` ↔ `gradeSubmission` seam wouldn't change.
- **The `checker` is a second eval surface, but trusted.** It's authored source shipped in the problem module, not user input, so it doesn't widen the untrusted surface — but it is a second place code is `new Function`'d, inside the same worker, and its runtime counts against the wall-clock budget.
...
- [`useAlgoSubmission`](../../src/problems/algo/useAlgoSubmission.ts) — the grading lifecycle: `run()` calls [`runTestsClient`](../../src/problems/algo/tester/runTestsClient.ts) (the browser-worker grader — no server round-trip) and stores the `SubmissionOutcome`; `runningMode` is which mode is in flight; `submittedSolution` is the answer-of-record, seeded from the progress store (`getEntry(id)?.solution`) and refreshed whenever a Submit passes (it also enables the "Last submission" button — build problems store no solution, so this is algo-only).
