...
- **Titles** — `loadPadTitle(id)` / `renamePad(id, title)`. The pad id (12-char hex) is the stable key; the title is an optional user-given display name. `renamePad` writes the trimmed title (blank clears it back to `undefined`) without touching files. Surfaced by the breadcrumb's [`EditablePadTitle`](../../src/pad/EditablePadTitle.tsx) (click to rename; Enter/blur commits, Escape cancels).
  - **Build problems** share pad storage (`padId` = problem id). [`BuildWorkspace`](../../src/problems/build/BuildWorkspace.tsx) passes `title={problem.title}` down through `CoderPad` → `PadWorkspace` → `usePadPersistence`, so the title rides the normal save (no mount effect) and shows in the breadcrumb **by name, not slug**. That title is fixed: editability is **structural**, not a stored flag — build problems render the plain `BuildToolbar`, only scratchpads render `PadToolbar` → `EditablePadTitle`, and the [`/pad/[id]`](../../src/app/pad/[id]/page.tsx) route **redirects any problem id to `/problems/[id]`**, so a build pad can never reach the editable scratchpad UI. See [navigation.md](navigation.md).
- [`clearPad(id)`](../../src/pad/pad.ts) — removes a pad from this browser. Backs both the Reset action (clear + reload rehydrates from the template) and the **Delete** action in [PadToolbar.tsx](../../src/pad/PadToolbar.tsx) (clear + `router.push("/pads")` back to the list). Delete is scratchpad-only — `BuildToolbar` offers Reset, not Delete, since a build problem isn't a user-created pad.
- [`listScratchPads()`](../../src/pad/pad.ts) — `listPads()` filtered by [`isScratchPadId`](../../src/pad/pad.ts) (a 12-hex `newPadId`), so problem-backed pads (build problems key the same store by their slug) don't appear. Drives the **`/pads` list page** ([PadsCatalog](../../src/pad/PadsCatalog.tsx)) via [`useScratchPads`](../../src/pad/useScratchPads.ts), which subscribes through the shared [`useCachedExternalStore`](../../src/lib/useCachedExternalStore.ts) (the `storage` event; the progress store uses the same hook), so a pad created in another tab shows up without a refresh. Same-tab writes don't fire `storage`; the list is good-enough-on-navigation.
